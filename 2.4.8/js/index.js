...
window.onload = function () {
  ...
  layui.use('form', function () {
    ...
    layui.form.on('switch(markpoints)', function (data) {
      if (this.checked) {
        ...
        var setAlert = layer.open({
          ...
          btn: ['确定'],
          shadeClose: true,
          content: $('.markpointsOption'),
          yes: function yes() {
            var fontsize = $('select[name=markpointsfontsize]').val();
            markPointsOption = {
              fontSize: fontsize,
              // 文字颜色
              color: fontcolor
  ...
  form.on('submit(formDemo)', function (data) {
    ...
    var option = {
      ...
      markPointsOption: markPointsOption,
      // markPointsOption: {
      //   fontSize: '20px',
      //   color: '#FFF'
      // },
